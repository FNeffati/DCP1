...
authors <- authors[which(log(authors$avg_rating) != -Inf),]

write.csv(authors, "/Users/joshuaingram/Main/Projects/DCP1/statistical_analysis/authors.csv")
```
...
```{r}
# average rating
ggplot(data = authors[which(authors$avg_rating > 20*mean(authors$avg_rating)),], aes(x = avg_rating)) +
...
mu_hat <- mean(log(authors$avg_rating))
sd_hat <- sd(log(authors$avg_rating))

ks.test(authors$avg_rating, "plnorm", meanlog = mu_hat, sdlog = sd_hat)
dlnorm(authors$avg_rating, meanlog = mu_hat, sdlog = sd_hat)
```
```{r}
x <- seq(0, max(authors$avg_rating), length = 1000)
fit_params <- fitdistr(authors$avg_rating, "lognormal")

fit <- dlnorm(x, fit_params$estimate["meanlog"], fit_params$estimate["sdlog"])

plot(x, fit, typle = "l", ylab = "Density", xlab = "X")
```
```{r}
ggplot(data = authors, aes(x = log(avg_rating))) + 
  geom_histogram(aes(y = ..density..), bins = 30, fill = "lightblue", color = "black") + 
  labs(x = "log(Surface Flux)", y = "Count", title = "Distribution of Beta Ceti Line Surface Fluxes", subtitle = "HST STIS Coadded Data") + 
  theme_bw() +
  stat_function(fun = dlnorm,
                args = list(meanlog = mean(log(authors$avg_rating)),
                            sdlog = sd(log(authors$avg_rating))),
                col = "green",
                size = 2)
```
```{r}
ggplot(data = authors, aes(x = log(avg_rating))) + 
  ...
  labs(x = "Radial Velocity", y = "Count") + 
  theme_bw() +
  stat_function(fun = dnorm,
                 args = list(mean = mean(log(authors$avg_rating)),
                             sd = sd(log(authors$avg_rating))),
                 col = "blue",
                 size = 2) + theme(text = element_text(size=12))
```
...
ggplot(data = authors, aes(x = avg_posts_year)) +
  ...
  labs(x = "Average Posts per Year", y = "Count", title = "Distribution of Author Yearly Posts") +
  theme_bw() +
...
ggplot(data = authors, aes(x = total_posts)) +
  ...
  labs(x = "Total Posts", y = "Count", title = "Distribution of Author Total Posts") +
  theme_bw() +
  theme(text = element_text(size = 20))
```
...
ggplot(data = authors, aes(x = avg_word_count)) +
  geom_histogram(bins = 75) +
  labs(x = "Average Word Count", y = "Count", title = "Distribution of Average Story Word Count") +
  theme_bw() +
  theme(text = element_text(size = 20))
```
...
```{r}
mean(authors$avg_rating)

# create bar plot for 5 groupings of story length
summary(authors$avg_rating)

authors %>% group_by(Quartile = ntile(avg_word_count, 6)) %>% 
  summarise(lower_val = min(avg_word_count), mean_avg_rating = mean(avg_rating))

authors %>% group_by(Quartile = ntile(total_posts, 10)) %>% 
  summarise(lower_val = min(total_posts), mean_avg_rating = mean(avg_rating))
```
